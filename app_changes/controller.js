...
return{
	get_all_tuna: function(req, res){
		...
		var channel = fabric_client.newChannel('educhain-channel');
		var peer = fabric_client.newPeer('grpcs://mainorgpeer1.test.hyperledgers.co.in', {'pem': Buffer.from(serverCert).toString()});
	...
	add_tuna: function(req, res){
		...
		var peer = fabric_client.newPeer('grpcs://mainorgpeer1.test.hyperledgers.co.in', {'pem': Buffer.from(serverCert).toString()});
		channel.addPeer(peer);
		var order = fabric_client.newOrderer('grpcs://orderer1.test.hyperledgers.co.in', {'pem': Buffer.from(serverCert).toString()});
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    if (isProposalGood) {
		        ...
		        event_hub.setPeerAddr('grpcs://mainorgpeer1.test.hyperledgers.co.in', {'pem': Buffer.from(serverCert).toString()});
