...
export CORE_PEER_ADDRESS="$ORG"-peer1-edu-peer."$ORG".svc.cluster.local:7051
cd /artifacts
...
peer channel fetch 0 -o educhain1-orderer.orderer.svc.cluster.local:7050 -c educhain-channel
peer channel join -b  educhain-channel_0.block
...
export ANCHORPATH=".channel_group.groups.Application.groups."$ORG"MSP.values.AnchorPeers"
ADDVALUE=`cat anchorpeer.json`
jq "$ANCHORPATH=$ADDVALUE" config.json > modified_config.json
